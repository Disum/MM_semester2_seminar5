...
int main(int argc, char **argv)
{
	...
	{
		fprintf(stderr, "Usage: %s m n [file_b] [file_a]\n", argv[0]);
		return ERR_INPUT;
	}
	...
	if( !(b = malloc(n*sizeof(double))) )
	{
		fprintf(stderr, "Not enugh memory!\n");
		free(a);
		return ERR_MEMORY;
	}
	if( !(c = malloc(m*sizeof(double))) )
	{
		fprintf(stderr, "Not enugh memory!\n");
		free(a);
		free(b);
	...
	if( name_b )
	{
		...
		res_read = read_matrix(b, n, 1, name_b);
